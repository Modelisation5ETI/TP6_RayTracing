                      ...
                      vec3 const& p_camera)
{
    ...
    //
    // c = (k_ambiant + k_diffus)*c0 + k_specular*b
    //
    // ********************************************* //
    ...
    float Ka = shading.ambiant;
    float Kd = shading.diffuse;
    float Ks = shading.specular;
    int e_s = shading.specular_exponent;

    float k_ambiant = Ka;

    float k_diffuse = Kd * std::max( 0.0f, dot( normal, d ) );

    float k_specular = Ks * pow(std::max(0.0f, dot(s,t)), e_s);


    color sourceColor = color(1.0f,1.0f,1.0f);
